	...
	*/
	...
	if (!isPure && !has(IteratorPrototype, ITERATOR)) hide(IteratorPrototype, ITERATOR, returnThis);
	...
	if (location.href.indexOf('/search/results') !== -1) {
	  ...
	  var regexp$1 = /搜索到\s+(\d+)\s+条\s+"(.+)"/;

	  var _regexp$exec = regexp$1.exec(searchResultText),
